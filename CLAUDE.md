...
|---|---|---|
| Public Safety | Police Incidents, Fire Stations, Fire Response Zones, Hydrants, School Crossing Guards, School Crosswalks, Cameras (511 + YouTube + FAA, see below) | All static (Cameras is static-ish, rebuilt by hand), City of Thunder Bay open data + CityProtect |
| Nature & Trails | Trees (37,752 records, ~9MB — the one dataset that matters most to keep lazy), Trails (OSM/Overpass), Watercourses (2,770 segments, filterable by type) | Static |
| Explore | Heritage Properties, Justthemenu.ca (restaurants), Waste Disposal Sites (26 records, colored by active/closed status), Municipal Wards (7 records) | Static; heritage/restaurants both went through real geocoding-precision problems, see README's "Known limitations" |
| Transportation | Street Signs (22,066 records, filterable by category), Rail Network (1,511 segments, filterable by track class), Road Right-of-Way (1,291 segments, filterable by designated width), Live Flights, Live Ships, Live Buses | Signs/rail/road-row are static-ish (rebuilt by hand); flights/ships/buses are the only three live layers |
